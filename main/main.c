...
static void bt_hf_hdl_stack_evt(uint16_t event, void *p_param)
{
    ...
    switch (event)
    {
        case APP_EVT_STACK_UP:
        {
            /* set up device name */
            char *dev_name = "ESP_HFP_AG_DISCOVER";
        ...
        case APP_EVT_UART_INIT:
        {
        	bt_app_uart_init();
        	break;
        }
        case APP_EVT_START_DISCOVERY:
        {
...
void app_main(void)
{
    ...
    bt_app_work_dispatch(bt_hf_hdl_stack_evt, APP_EVT_STACK_UP, NULL, 0, NULL);
    /* uart init */
    bt_app_work_dispatch(bt_hf_hdl_stack_evt, APP_EVT_UART_INIT, NULL, 0, NULL);
